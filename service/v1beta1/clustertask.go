package v1beta1

import (
	"context"
	"fmt"

	"github.com/hongyuxuan/tekton-sdk-go/config"
	"github.com/hongyuxuan/tekton-sdk-go/service"
	"github.com/hongyuxuan/tekton-sdk-go/types"
	"github.com/imroc/req/v3"
	tektonv1beta1 "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1beta1"
	"gopkg.in/yaml.v2"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

type ClusterTask struct {
	svcCtx     *service.ServiceContext
	httpclient *req.Client
	config     *config.Config
	token      string
}

func NewClusterTask(c *config.Config, namespace string, svcCtx *service.ServiceContext) *ClusterTask {
	token, err := svcCtx.GetBearerToken(namespace)
	if err != nil {
		panic(err)
	}
	return &ClusterTask{
		svcCtx:     svcCtx,
		httpclient: c.Httpclient,
		config:     c,
		token:      token,
	}
}

type ListClusterTaskResponse struct {
	ApiVersion string                      `json:"apiVersion"`
	Items      []tektonv1beta1.ClusterTask `json:"items"`
}

// https://apiserver.cluster.local:6443/apis/tekton.dev/v1beta1/clustertasks?labelSelector=app.kubernetes.io%2Fversion%3D0.3&limit=500
func (t *ClusterTask) List(ctx context.Context, opts metav1.ListOptions) (resp []tektonv1beta1.ClusterTask, err error) {
	req := t.httpclient.Get("/apis/tekton.dev/v1beta1/clustertasks").SetBearerAuthToken(t.token)
	if opts.LabelSelector != "" {
		req.SetQueryParam("labelSelector", opts.LabelSelector)
	}
	if opts.FieldSelector != "" {
		req.SetQueryParam("fieldSelector", opts.FieldSelector)
	}
	if opts.Limit > 0 {
		req.SetQueryParam("limit", fmt.Sprintf("%d", opts.Limit))
	} else {
		req.SetQueryParam("limit", "500") // default 500
	}
	var res ListClusterTaskResponse
	if err = req.SetSuccessResult(&res).Do(ctx).Err; err != nil {
		return
	}
	return t.processItems(res.Items), nil
}

// https://apiserver.cluster.local:6443/apis/tekton.dev/v1beta1/clustertasks/:name
func (t *ClusterTask) Get(ctx context.Context, name string) (resp tektonv1beta1.ClusterTask, err error) {
	if err = t.httpclient.Get(fmt.Sprintf("/apis/tekton.dev/v1beta1/clustertasks/%s", name)).
		SetBearerAuthToken(t.token).
		SetSuccessResult(&resp).
		Do(ctx).Err; err != nil {
		return
	}
	return
}

func (t *ClusterTask) GetYaml(ctx context.Context, name string) (string, error) {
	var task types.TektonResource
	if err := t.httpclient.Get(fmt.Sprintf("/apis/tekton.dev/v1beta1/clustertasks/%s", name)).
		SetBearerAuthToken(t.token).
		SetSuccessResult(&task).
		Do(ctx).Err; err != nil {
		return "", err
	}
	delete(task.Metadata.Annotations, "kubectl.kubernetes.io/last-applied-configuration")
	manifest, _ := yaml.Marshal(task)
	return string(manifest), nil
}

func (t *ClusterTask) Delete(ctx context.Context, name string) (err error) {
	return t.httpclient.Delete(fmt.Sprintf("/apis/tekton.dev/v1beta1/clustertasks/%s", name)).
		SetBearerAuthToken(t.token).
		Do(ctx).Err
}

func (t *ClusterTask) Create(ctx context.Context, yamlStr string) (err error) {
	return t.svcCtx.ApplyYaml(ctx, "", yamlStr, "ClusterTask")
}

func (t *ClusterTask) processItems(items []tektonv1beta1.ClusterTask) []tektonv1beta1.ClusterTask {
	for i := range items {
		delete(items[i].ObjectMeta.Annotations, "kubectl.kubernetes.io/last-applied-configuration")
		items[i].ObjectMeta.ManagedFields = nil
	}
	return items
}
